...
export async function generateCampaignPDF(data: PDFData): Promise<Blob> {
  ...
  doc.setFontSize(7);
  doc.text(
    "Generated by FlowFOR Creative powered by Google Gemini AI. All content should be reviewed before publishing.",
    margin,
    footerY + 5
  );
  doc.setTextColor(purple);
  ...
  for (let i = 2; i <= pageCount; i++) {
    ...
    doc.setFontSize(6);
    doc.text(
      "Generated by FlowFOR Creative powered by Google Gemini AI",
      margin,
      footerYPos + 4
    );
    doc.setTextColor(purple);
